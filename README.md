...
The starting script is `stateless_tcp_crypto_service.py` which starts the CS as a TCP server.
Without grmain the script can be started as mentionned below:
```
python3 stateless_tcp_crypto_service.py 
```

The purpose of Gramine is to make possible the execution of the python interpretor into an enclave. 
The SGX enclave is generated by the `Makefile` which takes as input the  `python.manifest.template`. 
The output is a `python.manifest.sgx` file that represents the enclave. 
This can be activated using `gramine-sgx ./python`. 
That is in our case:

```
cd pylurk/tests/stateless_tcp_crypto_service
make SGX=1 DEBUG=1
gramine-sgx stateless_tcp_crypto_service.py
```

Alternatively, it is also possible to run the CS with gramine but not in an SGX enclave as follws:
