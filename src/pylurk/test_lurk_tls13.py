...
## takes a lot of time
SERVER_LURK_MESSAGE_RESUMPTION_LOOP = True
...
eph_provided_req = { 'ephemeral_method': 'secret_provided', 'key': shared_secret }
ctx_struct = {'_status' : 'request'}
test_struct( Ephemeral, eph_provided_req, ext_title='secret_provided', ctx_struct=ctx_struct )

eph_provided_resp = { 'ephemeral_method': 'secret_provided', 'key': None } # empty in response
ctx_struct = {'_status' : 'success'}
test_struct( Ephemeral, eph_provided_resp, ext_title='secret_provided', ctx_struct=ctx_struct )


## Ephemeral (secret_generated)
  ## TLS engine requests the generation
eph_gen_req = { 'ephemeral_method': 'secret_generated', 'key': None } 
ctx_struct = {'_status' : 'request'}
test_struct( Ephemeral, eph_gen_req, ext_title="secret_generated", ctx_struct=ctx_struct )
...
keyshare_entry = { 'group' : 'secp256r1', 'key_exchange' : key_exchange }
  ## CS response
eph_gen_resp = { 'ephemeral_method': 'secret_generated', 'key': keyshare_entry } 
ctx_struct = {'_status' : 'success'}
test_struct( Ephemeral, eph_gen_resp, ext_title="secret_generated", ctx_struct=ctx_struct )
...
def init_cert_verify_handshake_list( ephemeral_mode:str, role='server') -> list:
  ...
  if ephemeral_mode == 'secret_generated': # generated by cs
...
def init_cert_verify_request_list( sig_algo: str ='ed25519', role='server', \
  ...
  for role in role_list:
    if role == 'server':
      eph_list = [ eph_gen_req, eph_provided_req ]
    elif role == 'client':
      eph_list = [ eph_provided_req ]
...
def init_cert_verify_response_list( sig_algo: str ='ed25519', role='server'):
  ...
  if role == 'server':
    eph_list = [ eph_gen_resp, eph_provided_resp ]
  elif role == 'client':
    eph_list = [ eph_provided_resp ]
...
def s_hand_and_app_handshake_list( ephemeral_mode ) -> list:
  ...
  hs = [ hs_server_hello, hs_encrypted_extensions, hs_certificate_request ]
...
def s_hand_and_app_secret_request_list( ):
  ...
  req_list = []
  for last_exchange in [ True, False ]:
    for ephemeral in [ eph_no, eph_provided_req, eph_gen_req ]:
      ephemeral_method  = ephemeral[ 'ephemeral_method' ]
...
for last_exchange in [ True, False ]:
  for ephemeral in [ eph_no, eph_gen_resp ]:
    s_hand_and_app_resp = {\
      'tag' : { 'last_exchange' : last_exchange }, 
      'session_id' : token_bytes (4 ), 
      'ephemeral' : ephemeral, 
...
#                                   tls13_conf=None, last_exchange=None ):
    ...
    for req in init_cert_verify_request_list( sig_scheme, role='server', \
                 tls13_conf=conf, last_exchange=None ):
      session = SSession( conf ) 
      init_cert_verify_test( req, 'request', role='server' )
      resp = session.serve( req, 's_init_cert_verify', 'request')
      init_cert_verify_test( resp, 'success', role='server' )
...
#  conf = configure( sig_algo ) 
  for req in s_new_ticket_request_list():
    ...
    s_init_cert_verify_req = init_cert_verify_request_list( sig_scheme, \
                               role='server', tls13_conf=conf, \
                               last_exchange=False )[ 0 ]
...
def session_resumption( conf, s_init_cert_verify_req, \
  ...
  s_init_cert_verify_req[ 'tag' ]['last_exchange' ] = False
  init_cert_verify_test( s_init_cert_verify_req, 'request', role='server' )
  session = SSession( conf )
  s_init_cert_verify_resp = session.serve( s_init_cert_verify_req, 's_init_cert_verify', 'request')
  init_cert_verify_test( s_init_cert_verify_resp, 'success', role='server' )
...
if SERVER_PAYLOAD_SESSION_RESUMPTION_LOOP == True:
  for sig_scheme in sig_scheme_list:
    ...
    for s_init_cert_verify_req in  init_cert_verify_request_list( sig_scheme,\
...
##  tls13_conf = configure(sig_scheme=sig_scheme, role='server')
  ...
  s_init_cert_verify_req = init_cert_verify_request_list( sig_scheme, role='server', tls13_conf=conf.conf[ ( 'tls13', 'v1' ) ], last_exchange=False )[0]
  s_new_ticket_session_req = s_new_ticket_request_list( )[ 0 ] 
  s_init_early_secret_req = s_init_early_secret_request_list()[ 0 ]
  s_hand_and_app_secret_req = s_hand_and_app_secret_request_list( )[0]
  s_new_ticket_session_req2 = s_new_ticket_request_list( )[ 0 ] 
...
##    conf = configure( sig_scheme=sig_scheme, role='server' )
    ...
    for s_init_cert_verify_req in init_cert_verify_request_list( sig_scheme, role='server', tls13_conf=conf.conf[ ( 'tls13', 'v1' ) ], last_exchange=False ):
#      init_cert_verify_request_list( sig_scheme, tls13_conf=tls13_conf, last_exchange=False ):
      for s_new_ticket_session_req in  s_new_ticket_request_list():
        for s_init_early_secret_req in s_init_early_secret_request_list():
          for s_hand_and_app_secret_req in s_hand_and_app_secret_request_list():
            for s_new_ticket_session_req2 in s_new_ticket_request_list():
