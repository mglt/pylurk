...
class TlsHandshake:
  ...
  def key_share_index( self, side ):
    ...
    pointers are returned (instead of simply the value) as we also want to update the key_share value.
    """
    if side == 'server':
...
##  def update_key_share( self, server_key_exchange ) -> NoReturn:
  ...
  def truncated_client_hello( self, ticket_info_list ) -> bytes :
    ...
    for ticket_info in ticket_info_list:
      binder_len = pylurk.tls13.crypto_suites.hash_sanity_check( ticket_info[ 'tls_hash' ] ).digest_size
    ...
    ch_index = self.latest_client_hello_index( )
    self.msg_list[ ch_index ][ 'data' ][ 'extensions' ][ -1 ][  'extension_data'] [ 'binders' ] = binder_list  
...
#      test_vector.handle_bin( 'truncated_client_hello' , truncated_client_hello_bytes ) 
  ...
  def update_binders( self, ticket_info_list, binder_finished_key_list )-> None :
    ...
    for ticket_info in ticket_info_list :
      ...
      binder = self.compute_binder( tls_hash, binder_finished_key, truncated_client_hello, binder_index=ticket_info_list.index( ticket_info ) )
